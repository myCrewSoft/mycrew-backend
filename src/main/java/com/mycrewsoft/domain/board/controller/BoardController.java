...
public class BoardController {
	...
	@GetMapping
	public ResponseEntity<ApiResponse<Page<BoardResponse>>> getRes(
			@Validated @ModelAttribute BoardSearchRequest searchRequest 
	){
		// 1. 서비스의 페이징 메서드(getBoard)를 호출하고 Page 객체로 받습니다.
		Page<BoardResponse> boardPage = service.getBoard(searchRequest);
		
		// 2. 응답 데이터 타입을 Page<BoardResponse>로 일치시켜 전송합니다.
