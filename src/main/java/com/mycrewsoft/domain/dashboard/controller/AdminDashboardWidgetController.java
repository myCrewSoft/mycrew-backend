...
public class AdminDashboardWidgetController {
    ...
    public ResponseEntity<ApiResponse<AdminAttendanceWidgetResponse>> readAttendanceWidget() {
        return ResponseEntity.ok(ApiResponse.success(adminDashboardWidgetService.readAttendanceWidget()));
    }

    @Operation(summary = "중요 일정 위젯",
            description = "오늘 및 다가오는 전사·간부 일정을 통합하여 조회합니다. ADMIN_CONSOLE_ACCESS 권한이 필요합니다.")
    @GetMapping("/schedule")
    public ResponseEntity<ApiResponse<AdminImportantScheduleWidgetResponse>> readImportantScheduleWidget() {
        return ResponseEntity.ok(ApiResponse.success(adminDashboardWidgetService.readImportantScheduleWidget()));
    }

    @Operation(summary = "프로젝트 현황 위젯",
            description = "전체 프로젝트의 상태별(예정/진행 중/완료/중단) 집계 통계를 조회합니다. ADMIN_CONSOLE_ACCESS 권한이 필요합니다.")
    @GetMapping("/project")
    public ResponseEntity<ApiResponse<AdminProjectStatusWidgetResponse>> readProjectStatusWidget() {
        return ResponseEntity.ok(ApiResponse.success(adminDashboardWidgetService.readProjectStatusWidget()));
    }

    @Operation(summary = "공지사항 위젯",
            description = "현재 등록된 공지사항 중 최신 5개를 조회합니다. ADMIN_CONSOLE_ACCESS 권한이 필요합니다.")
    @GetMapping("/notice")
    public ResponseEntity<ApiResponse<AdminNoticeWidgetResponse>> readNoticeWidget() {
        return ResponseEntity.ok(ApiResponse.success(adminDashboardWidgetService.readNoticeWidget()));
    }
}
